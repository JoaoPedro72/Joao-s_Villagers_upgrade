...
execute if score 20 j.ticks matches 1 if score @s j_vida_atual <= 10 j.num run rotate @s ~180 ~


$execute unless entity @n[tag=$(id).target,distance=..10] run function j_ai:move

$execute if entity @n[tag=$(id).target,distance=..8] run function j_ai:esquiva
