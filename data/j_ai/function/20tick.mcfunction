...
execute as @e[tag=AI] at @s unless block ~ ~-0.5 ~ #j_data:atravessavel unless score @s j.jump >= j.cooldown.pulo j.num run scoreboard players add @s j.jump 1
execute as @e[tag=AI] at @s unless block ~ ~-0.5 ~ #j_data:atravessavel if score @s j.jump > j.cooldown.pulo j.num run scoreboard players remove @s j.jump 1
